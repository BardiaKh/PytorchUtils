...
class BKhModule(pl.LightningModule):
    ...
    def train_dataloader(self):
        if self.train_ds is None:
            ...
            self.train_dl = torch.utils.data.DataLoader(self.train_ds, batch_size=self.batch_size, sampler=instance_sampler, shuffle=True if instance_sampler is None else False, num_workers=self.dl_workers, collate_fn=self.collate_fn, pin_memory=self.pin_memory, drop_last=True, prefetch_factor=None if self.dl_workers==0 else 1)
    ...
    def val_dataloader(self):
        if self.val_ds is None:
            ...
            self.val_dl = torch.utils.data.DataLoader(self.val_ds, batch_size=self.val_batch_size, sampler=instance_sampler, shuffle=False, num_workers=self.dl_workers, collate_fn=self.val_collate_fn, pin_memory=self.pin_memory, drop_last=False, prefetch_factor=None if self.dl_workers==0 else 1)
            return self.val_dl
