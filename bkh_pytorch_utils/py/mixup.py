...
Added the following features:
 - Support for already one-hot encoded targets
"""
import numpy as np
import torch
...
def mixup_target_one_hot(target_one_hot, lam=1., smoothing=0.0):
    num_classes = target_one_hot.shape[-1]
    off_value = smoothing / num_classes
    on_value = 1. - smoothing + off_value
    y1 = target_one_hot * on_value + off_value
    y2 = target_one_hot.flip(0) * on_value + off_value
    return y1 * lam + y2 * (1. - lam)
